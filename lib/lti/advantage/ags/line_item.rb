...
module Lti
  module Advantage
    module AGS
      class LineItem
        TIMEZONE_TIMESTAMP_FORMAT = /(?:Z|[+-]\d{2}:\d{2})\z/.freeze
        ...
        def self.stringify_keys(hash)
          Hash(hash).each_with_object({}) { |(key, value), memo| memo[key.to_s] = value }
        end
        ...
        def validate!
          ...
          raise ValidationError, "gradesReleased must be boolean" unless grades_released.nil? || [true, false].include?(grades_released)
