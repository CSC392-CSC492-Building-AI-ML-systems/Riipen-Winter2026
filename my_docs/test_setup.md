...
docker start dory_dinghy_http_proxy

to see inside docker: docker exec -it canvas-lms-web-1 bash

open http://canvas.docker

## start the test website

1. run `hostname -i` to check your local ip
2. change the following part to your ip.
```
# CHANGE THIS TO YOUR LOCAL IP
TOOL_HOST = "192.168.2.92:4567"
LMS_ISSUER = "http://192.168.2.92:3000" # If needed for server-to-server
```
3. run `bundle exec ruby demo/app.rb`

you can then test it by trying the following in your browser

http://192.168.2.92:4567/lti/launch
  => Sinatra doesn’t know this ditty.

http://192.168.2.92:4567/oidc/init
  => this should say like `Validation Error: iss (issuer) is missing`, since we are
  suppose to open it by redirection from canvas
...
## config the LTI in canvas
  ...
  In the "Admin" -> "Developer Keys" -> "+ Developer Key" -> "LTI Key" page, set the
    ...
    * Title: My Test Tool
    * Target Link URI: http://<your-ip>:4567/lti/launch
    * OpenID Connect Initiation Url: http://<your-ip>:4567/oidc/init
    * JWKS Method: Choose "Public JWK"
    * Public JWK: copy the JSON from http://<your-ip>:4567/lti/jwk

  2. Redirect URIs
  In the Redirect URIs field (usually at the top of the form), you must put the final launch
  URL:
   * http://<your-ip>:4567/lti/launch
